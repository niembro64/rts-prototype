...
export function renderFactory(ctx: BuildingRenderContext): void {
  const { graphics, entity, left, top, width, height, playerColor, sprayParticleTime, detail } = ctx;
  ...
  if (detail) {
    ...
    drawGear(graphics, left + width * 0.25, top + height * 0.35, 12, gearPhase, playerColor);
    drawGear(graphics, left + width * 0.75, top + height * 0.35, 10, -gearPhase * 1.3, playerColor);
    drawGear(graphics, left + width * 0.5, top + height * 0.6, 14, gearPhase * 0.8, playerColor);
    ...
    for (let i = -1; i < conveyorWidth / 8 + 1; i++) {
      ...
      if (lineX >= conveyorX && lineX <= conveyorX + conveyorWidth) {
        graphics.lineBetween(lineX, conveyorY, lineX, conveyorY + conveyorHeight);
      }
    }
    ...
    graphics.fillCircle(left + lightMargin, top + lightMargin, lightRadius);
    ...
    graphics.fillCircle(left + width - lightMargin, top + lightMargin, lightRadius);
  ...
  if (isProducing && factory.buildQueue.length > 0) {
    ...
    // Queue indicator (small dots for queued items) — detail only
    if (detail) {
