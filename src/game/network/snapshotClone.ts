...
function clearEntitySnapshotWireSource(source: EntitySnapshotWireSource): void {
  ...
  source.basicTypedPlaceholderRows = 0;
  source.unitTypedPlaceholderRows = 0;
  source.buildingTypedPlaceholderRows = 0;
...
function copyFactoryInto(src: ReusableFactory, dst: ReusableFactory): ReusableFactory {
  ...
  if (srcRoute !== null) {
    ...
    route.length = srcRoute.length;
    for (let i = 0; i < srcRoute.length; i++) {
      route[i] = copyWaypointInto(srcRoute[i], route[i] ?? createWaypointDto());
    }
  } else {
    dst.route = null;
  }
...
export class ReusableNetworkSnapshotCloner {
  ...
  private serverMeta: NonNullable<NetworkServerSnapshot['serverMeta']> = {
    ...
    shieldPowerPlayerMask: undefined,
  ...
  clear(): void {
    ...
    this.snapshot.entityDeltaOnly = undefined;
