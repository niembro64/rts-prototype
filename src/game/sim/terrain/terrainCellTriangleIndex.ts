...
/** The inclusive cell range one triangle's XZ bounding box covers. Both the
 *  counting pass and the fill pass below walk the exact same ranges, so the
 *  arithmetic lives here once — a drift between the two passes would size the
 *  offset table for one set of cells and write into another. Writes into a
 *  caller-owned scratch so neither pass allocates per triangle. */
function triangleCellRange(
  input: TerrainCellTriangleIndexInput,
  tri: number,
  out: TriangleCellRange,
): void {
  const { cellsX, cellsY, cellSize, vertexCoords, triangleIndices } = input;
  ...
  out.minCellX = clampCell(Math.floor(Math.min(ax, bx, cx) / cellSize), cellsX);
  out.maxCellX = clampCell(Math.floor(Math.max(ax, bx, cx) / cellSize), cellsX);
  out.minCellY = clampCell(Math.floor(Math.min(az, bz, cz) / cellSize), cellsY);
  out.maxCellY = clampCell(Math.floor(Math.max(az, bz, cz) / cellSize), cellsY);
}

function buildTerrainCellTriangleIndexTs(
  input: TerrainCellTriangleIndexInput,
): TerrainCellTriangleIndex {
  const { cellsX, triangleIndices } = input;
  const cellCount = Math.max(0, Math.floor(cellsX * input.cellsY));
  const cellTriangleCounts = new Array<number>(cellCount).fill(0);
  const range: TriangleCellRange = {
    minCellX: 0,
    maxCellX: 0,
    minCellY: 0,
    maxCellY: 0,
  ...
  for (let tri = 0; tri < triangleIndices.length / 3; tri++) {
    triangleCellRange(input, tri, range);
