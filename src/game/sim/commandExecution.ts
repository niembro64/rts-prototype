...
import { BAR_NEAREST_QUEUE_INSERT_INDEX } from './commands';
import type {
  CombatFireState,
  Entity,
  EntityId,
...
import {
  ...
  clampToMap,
  computeSlowestFormationSpeedFactors,
  maxFormationUnitRadius,
...
function offsetQueueInsertIndex(queueInsertIndex: number | undefined, offset: number): number | undefined {
  if (queueInsertIndex === undefined || queueInsertIndex === BAR_NEAREST_QUEUE_INSERT_INDEX) {
    return queueInsertIndex;
  }
  return queueInsertIndex + offset;
}
...
function resetAirborneLoiterToCurrentPosition(entity: Entity, world: WorldState): void {
  ...
  unit.airborneLoiterTargetZ = Number.isFinite(entity.transform.z)
    ? entity.transform.z
    : world.getGroundZ(x, y);
  unit.airborneLoiterTurnSign = null;
}
...
function executeMoveCommand(ctx: CommandContext, command: MoveCommand): void {
  ...
  // First pass: collect valid units once for formation layout.
  ...
  const unitCount = validUnits.length;
...
function executeStopCommand(ctx: CommandContext, command: StopCommand): void {
  for (let i = 0; i < command.entityIds.length; i++) {
    ...
    if (entity.unit !== null) {
      ...
      resetAirborneLoiterToCurrentPosition(entity, ctx.world);
...
function executeWaitCommand(ctx: CommandContext, command: WaitCommand): void {
  ...
  for (let i = 0; i < units.length; i++) {
    ...
    if (command.queue) {
      addQueuedActionToUnit(entity, action, queueFront, queueInsertIndex, ctx.world);
    } else {
...
function executeRepairAreaCommand(ctx: CommandContext, command: RepairAreaCommand): void {
  ...
  const targets = prepareBarEntityAreaTargets(findRepairAreaTargets(
    ctx,
    commander,
    command.targetX,
    command.targetY,
...
function executeReclaimAreaCommand(ctx: CommandContext, command: ReclaimAreaCommand): void {
  ...
  const targets = prepareBarReclaimAreaTargets(findReclaimAreaTargets(
    ctx,
    commander,
    command.targetX,
    command.targetY,
...
function barSplitIncludesTarget(
  targetIndex: number,
  targetCount: number,
  sourceIndex: number,
  sourceCount: number,
): boolean {
  // Direct zero-based translation of cmd_area_commands_filter.lua's
  // splitTargets loop (whose indices are one-based).
  const targetOrdinal = targetIndex + 1;
  const sourceOrdinal = sourceIndex + 1;
  return targetOrdinal % targetCount === sourceOrdinal % targetCount ||
    sourceOrdinal % sourceCount === targetOrdinal % sourceCount;
}

function prepareBarEntityAreaTargets(
  targets: Entity[],
  command: BarAreaExpansionCommand,
  commandSource: Entity,
  targetsAlreadySortedById = false,
): Entity[] {
  const splitIndex = command.targetSplitIndex;
  const splitCount = command.targetSplitCount;
  if (splitIndex !== undefined && splitCount !== undefined) {
    if (!targetsAlreadySortedById) targets.sort((a, b) => a.id - b.id);
    const splitTargets: Entity[] = [];
    for (let i = 0; i < targets.length; i++) {
      if (barSplitIncludesTarget(i, targets.length, splitIndex, splitCount)) {
...
function prepareBarReclaimAreaTargets(
  ...
  if (splitIndex !== undefined && splitCount !== undefined) {
    ...
    const splitTargets: ReclaimTarget[] = [];
    for (let i = 0; i < targets.length; i++) {
      if (barSplitIncludesTarget(i, targets.length, splitIndex, splitCount)) {
        splitTargets.push(targets[i]);
      }
...
function enqueueAreaTargetActionsInOrder<T>(
  ...
  for (let i = 0; i < targets.length; i++) {
    enqueue(
      ...
      queue || i > 0,
      false,
      offsetQueueInsertIndex(queueInsertIndex, i),
    );
  }
}
...
function executeAttackAreaCommand(ctx: CommandContext, command: AttackAreaCommand): void {
  ...
  const sharedTargets = command.splitTargets === true
    ? targets.slice().sort((a, b) => a.id - b.id)
    : prepareBarEntityAreaTargets(targets, command, entities[0]);
  const splitSourceIndexById = command.splitTargets === true
    ? new Map<EntityId, number>()
    : null;
  if (splitSourceIndexById !== null) {
    for (let i = 0; i < command.entityIds.length; i++) {
      splitSourceIndexById.set(command.entityIds[i], i);
    }
  }
  ...
  for (let i = 0; i < entities.length; i++) {
    ...
    const assignedTargets = command.splitTargets === true
      ? prepareBarEntityAreaTargets(
          sharedTargets,
          {
            targetSplitIndex: splitSourceIndexById?.get(entity.id) ?? 0,
            targetSplitCount: command.entityIds.length,
          },
          entity,
          true,
        )
    ...
    for (let targetIndex = 0; targetIndex < assignedTargets.length; targetIndex++) {
      if (entityCanBarAttackTarget(entity, assignedTargets[targetIndex])) {
        compatibleTargets.push(assignedTargets[targetIndex]);
      }
    }
    if (compatibleTargets.length === 0) {
      if (command.splitTargets === true) continue;
      executeMoveCommand(ctx, {
        type: 'move',
        tick: command.tick,
        entityIds: [entity.id],
...
function isBarSpatialQueueAction(action: UnitAction): boolean {
  // Mirrors cmd_commandinsert.lua:GetCommandPos. Targeted orders already
  // carry their target's position in UnitAction, so they participate just as
  // Spring target-id commands do after GetUnitOrFeaturePosition resolves them.
  switch (action.type) {
    case 'move':
    case 'fight':
    case 'build':
...
function barQueuePointDistance(
  ax: number,
  ay: number,
  az: number,
  bx: number,
  ...
  return DMath.hypot(ax - bx, ay - by, az - bz);
}

function barQueueActionPoint(action: UnitAction, world: WorldState | undefined): { x: number; y: number; z: number } {
  const target = action.targetId === undefined ? undefined : world?.getEntity(action.targetId);
  if (target !== undefined) {
    const point = getEntityTargetPoint(target);
    return { x: point.x, y: point.y, z: point.z };
  }
  return { x: action.x, y: action.y, z: action.z ?? 0 };
}

/** Select the insertion point with the smallest added route length, matching
 * BAR's Space+Shift CommandInsert widget. Non-spatial queue entries keep their
 * indices but do not become route anchors. */
function barNearestSpatialQueueInsertIndex(
  entity: Entity,
  action: UnitAction,
  world: WorldState | undefined,
): number {
  const unit = entity.unit;
  if (unit === null || !isBarSpatialQueueAction(action)) return unit?.actions.length ?? 0;

  const targetPoint = barQueueActionPoint(action, world);
  let previousX = entity.transform.x;
  let previousY = entity.transform.y;
  let previousZ = entity.transform.z;
  let bestAddedDistance = Number.POSITIVE_INFINITY;
  let bestIndex = unit.actions.length;

  for (let i = 0; i < unit.actions.length; i++) {
    const queued = unit.actions[i];
    if (!isBarSpatialQueueAction(queued)) continue;
    const queuedPoint = barQueueActionPoint(queued, world);
    const addedDistance =
      barQueuePointDistance(queuedPoint.x, queuedPoint.y, queuedPoint.z, targetPoint.x, targetPoint.y, targetPoint.z) +
      barQueuePointDistance(previousX, previousY, previousZ, targetPoint.x, targetPoint.y, targetPoint.z) -
      barQueuePointDistance(previousX, previousY, previousZ, queuedPoint.x, queuedPoint.y, queuedPoint.z);
    if (addedDistance < bestAddedDistance) {
      bestAddedDistance = addedDistance;
      bestIndex = i;
    }
    previousX = queuedPoint.x;
    previousY = queuedPoint.y;
    previousZ = queuedPoint.z;
  }

  const appendDistance = barQueuePointDistance(
    previousX,
    previousY,
    previousZ,
    targetPoint.x,
  ...
  return appendDistance < bestAddedDistance ? unit.actions.length : bestIndex;
}
...
function queuedActionInsertIndex(
  ...
  if (queueInsertIndex === BAR_NEAREST_QUEUE_INSERT_INDEX) {
    return barNearestSpatialQueueInsertIndex(entity, action, world);
  }
...
function addPatrolActionToUnit(
  ...
  if (world === undefined) {
    ...
    else addQueuedActionToUnit(entity, action, queueFront, queueInsertIndex, world);
    return;
  }
  ...
  if (hasExistingPatrol) {
    addQueuedActionToUnit(entity, action, queueFront, queueInsertIndex, world);
    return;
  }

  const index = queuedActionInsertIndex(entity, action, queueFront, queueInsertIndex, world);
