...
function _updateTravelingProjectilesJS(
  ...
  SIM_TICK_INSTRUMENTATION.phase('combat.projectiles.guidancePrepare');
  if (batchCount === 0) {
    SIM_TICK_INSTRUMENTATION.phase('combat.projectiles.guidance');
    SIM_TICK_INSTRUMENTATION.phase('combat.projectiles.integrate');
    SIM_TICK_INSTRUMENTATION.phase('combat.projectiles.scatter');
    return;
  }
  if (sim === undefined) {
