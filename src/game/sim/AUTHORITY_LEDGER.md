...
| Economy stockpile clamps and batched income/converter application | `rts-sim-wasm/src/lib.rs` `economy_*` helpers | `src/game/sim/economy.ts` packs income/converter rows, copies back player stockpiles, and records renderer-facing resource movement events |
