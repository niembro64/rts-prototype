...
| Economy, resource movement, producer stockpile accounting | `economy.ts`, `energyDistribution.ts`, `resourceMovement.ts` | Resource movement visuals can stay renderer-side; stockpile truth should migrate with economy state; wind oscillator, producer/converter per-player rate aggregation, converter tax transfer math, and shared stockpile credit/debit clamps are now Rust/WASM-owned |
