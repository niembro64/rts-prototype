...
| Packed/guided projectile motion and swept body-hit selection kernels | WASM projectile pool packed stepping, non-packed constant-acceleration integration, homing thrust, terrain-follow vertical thrust, reflector intersections, and `projectile_hitbox_sweep_batch` body/building/projectile hit selection | TypeScript still owns projectile entity lifecycle, homing target policy/intercept setup, D-gun terrain-height sampling, turret sub-hitbox composition, collision consequences, death events, and network spawn/despawn/velocity-update events |
