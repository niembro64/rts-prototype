...
| Packed/guided projectile motion, swept hit selection, and terminal classification kernels | WASM projectile pool packed stepping, non-packed constant-acceleration integration, homing thrust, terrain-follow vertical thrust, reflector intersections, reflected projectile response math, `projectile_hitbox_sweep_batch` body/building/projectile/current-tick turret sub-hitbox hit selection, and `projectile_terminal_consequence_batch` timeout/ground/water/reflector/HP/out-of-bounds consequence classification | TypeScript still owns projectile entity lifecycle, homing target policy/intercept setup, terrain/water input sampling, D-gun terrain-height sampling, returned terminal diff application, direct-hit/splash damage consequences, death events, and network spawn/despawn/velocity-update event application |
...
| Projectile lifecycle, homing policy, collision consequences, submunition spawn/despawn events | `combat/projectileSystem.ts`, `ProjectileCollisionHandler.ts` | Packed motion, reflector intersections, reflected projectile response math, swept body/building/projectile/turret sub-hitbox hit selection, submunition launch velocity scatter, and terminal consequence classification exist in WASM, but projectile consequence application, detonation payloads, and spawned/removed entity diffs are not fully Rust-owned |
