...
| Packed/guided projectile motion and swept hit selection kernels | WASM projectile pool packed stepping, non-packed constant-acceleration integration, homing thrust, terrain-follow vertical thrust, reflector intersections, reflected projectile response math, and `projectile_hitbox_sweep_batch` body/building/projectile/current-tick turret sub-hitbox hit selection | TypeScript still owns projectile entity lifecycle, homing target policy/intercept setup, D-gun terrain-height sampling, collision consequences, death events, and network spawn/despawn/velocity-update event application |
...
| Projectile lifecycle, homing policy, collision consequences, submunition spawn/despawn events | `combat/projectileSystem.ts`, `ProjectileCollisionHandler.ts` | Packed motion, reflector intersections, reflected projectile response math, and swept body/building/projectile/turret sub-hitbox hit selection exist in WASM, but projectile termination/consequence authority is not fully Rust-owned |
