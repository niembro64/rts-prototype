...
| --- | --- | --- |
| Unit arrival and flying-loiter control thrust | `arrivalControlStepBatch` computes the velocity-aware PD thrust for packed waypoint candidates; `flying_loiter_step_batch` computes idle/waiting flying-unit orbit steering for packed candidates | `Simulation.updateUnits` still owns action semantics, target sweeping, path refresh, loiter target storage, and queue advancement |
...
| Unit action queues, path-intent promotion, stuck detection, replanning, fight/guard/attack halt policy | `Simulation.updateUnits` plus `Pathfinder.ts` | Arrival math, flying-loiter steering, and build/repair/reclaim target-distance checks are already WASM-owned; queue semantics and per-unit control flow are still TypeScript |
