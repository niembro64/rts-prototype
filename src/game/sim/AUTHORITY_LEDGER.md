...
| Economy, resource movement, converter tax, producer stockpile accounting | `economy.ts`, `energyDistribution.ts`, `resourceMovement.ts` | Resource movement visuals can stay renderer-side; stockpile truth should migrate with economy state; wind oscillator and per-player wind production-rate aggregation are now Rust/WASM-owned |
