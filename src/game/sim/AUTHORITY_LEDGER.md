...
| Factory construction-site placement geometry | `rts-sim-wasm/src/lib.rs` `factory_build_spot` | `src/game/sim/factoryConstructionSite.ts` supplies authored footprint/radius config and entity/rally inputs, then maps the returned spot into TS objects |
...
| Economy stockpile clamps, batched income/converter application, and construction/repair equal-share debit allocation | `rts-sim-wasm/src/lib.rs` `economy_*` helpers | `src/game/sim/economy.ts` and `energyDistribution.ts` pack rows, copy back player stockpiles, and record renderer-facing resource movement events |
