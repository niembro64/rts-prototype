...
| Packed/guided projectile motion and swept hit selection kernels | WASM projectile pool packed stepping, non-packed constant-acceleration integration, homing thrust, terrain-follow vertical thrust, reflector intersections, and `projectile_hitbox_sweep_batch` body/building/projectile/current-tick turret sub-hitbox hit selection | TypeScript still owns projectile entity lifecycle, homing target policy/intercept setup, D-gun terrain-height sampling, collision consequences, death events, and network spawn/despawn/velocity-update events |
