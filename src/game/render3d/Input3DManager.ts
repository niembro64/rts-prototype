...
export class Input3DManager {
  ...
  /** True while the next left-click will issue a capture command. */
  ...
  /** The special-mode predicates and exits, shared verbatim by the mode-click
   *  controller and the hotkey controller. Both must see the same mode state
   *  or a key and a click disagree about which mode is active, so the wiring
   *  is written once here. */
  private specialModeAccessors() {
    return {
