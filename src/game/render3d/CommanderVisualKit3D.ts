...
export class CommanderVisualKit3D {
  ...
  buildKit(primaryMat: THREE.Material): THREE.Group {
    ...
    addBox(primaryMat, -0.28, 1.24, -0.38, 0.36, 0.07, 0.08);
    addBox(primaryMat, -0.28, 1.24, 0.38, 0.36, 0.07, 0.08);
    addBox(primaryMat, -0.38, 1.31, 0, 0.09, 0.22, 0.12);
  ...
  decorateTurret(
    ...
    const brow = new THREE.Mesh(this.boxGeom, primaryMat);
    ...
    if (tm.pitchGroup) {
      const sleeve = new THREE.Mesh(this.boxGeom, primaryMat);
    ...
    const crest = new THREE.Mesh(this.boxGeom, primaryMat);
