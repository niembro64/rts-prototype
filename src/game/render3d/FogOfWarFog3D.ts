...
import { FOG_CONFIG, cssHexToNumber } from '@/fogConfig';
...
export class FogOfWarFog3D {
  ...
  private spawnFog(x: number, y: number): boolean {
    ...
    const color = cssHexToNumber(this.profile.colorHex, 'fogConfig.fogOfWar.colorHex');
    ...
    const fog: FogSphere = {
      ...
      radius,
      maxAlpha: Math.max(0, this.profile.maxAlpha),
      threeX: x,
      threeY: z,
      threeZ: y,
      r,
