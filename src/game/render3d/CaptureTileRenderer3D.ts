      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace(
          ...
          [
            ...
            'float bladeMark = terrainBoxMark(bladeLocal, vec2(0.028, 0.36)) * step(0.30, bladeSeed);',
            'vec3 bladeRgb = mix(vec3(0.29, 0.39, 0.20), vec3(0.43, 0.49, 0.27), terrainHash12(bladeCell + vec2(7.7, 2.9)));',
            'terrainRgb = mix(terrainRgb, bladeRgb, bladeMark * flatDetail * 0.22);',
            'vec2 stickGrid = detailPos / 58.0;',
            ...
            'float stickMark = terrainBoxMark(stickLocal, vec2(0.018, 0.28)) * step(0.82, stickSeed);',
            'vec3 stickRgb = mix(terrainRgb, vec3(0.43, 0.36, 0.23), 0.48);',
            'terrainRgb = mix(terrainRgb, stickRgb, stickMark * flatDetail * 0.34);',
