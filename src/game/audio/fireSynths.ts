...
function laserZap(tk: AudioToolkit, speed: number, vol: number): void {
  const d = 0.12 / speed;
  playTone(tk, 'sawtooth', 300 * speed, 150 * speed, d, 0.2 * vol, 0.25 * vol, 0, 'lowpass', 1500);
}

// Minigun - short punchy noise burst + click
function minigun(tk: AudioToolkit, speed: number, vol: number): void {
...
function cannon(tk: AudioToolkit, speed: number, vol: number): void {
...
function shotgun(tk: AudioToolkit, speed: number, vol: number): void {
  for (let i = 0; i < 3; i++) {
...
function shieldFire(tk: AudioToolkit, speed: number, vol: number): void {
  // Deep bass pulse
