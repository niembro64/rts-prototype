...
function buildMovementSection(blueprint: UnitBlueprint): LoadingUnitInfoSection {
  ...
  const items: LoadingUnitInfoNode[] = [
    ...
    node('Route media', labelCase(runtime.type), undefined, [
      ...
      stat('Dry max slope', climb.maxSlopeDeg === null ? 'not contact-bound' : `${fmt(climb.maxSlopeDeg)} deg`),
      stat('Dry normal floor', climb.minGroundNormalZ === null ? 'none' : fmt(climb.minGroundNormalZ, 3)),
      stat('Full-water MOVE max bed slope', climb.maxWaterMoveSlopeDeg === null ? 'not bed-bound' : `${fmt(climb.maxWaterMoveSlopeDeg)} deg`),
      stat('Full-water MOVE normal floor', climb.minWaterMoveNormalZ === null ? 'none' : fmt(climb.minWaterMoveNormalZ, 3)),
      stat('Full-water WAYPOINT max bed slope', climb.maxWaterWaypointSlopeDeg === null ? 'not bed-bound' : `${fmt(climb.maxWaterWaypointSlopeDeg)} deg`),
      stat('Full-water WAYPOINT normal floor', climb.minWaterWaypointNormalZ === null ? 'none' : fmt(climb.minWaterWaypointNormalZ, 3)),
    ]),
