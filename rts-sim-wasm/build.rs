...
fn main() {
    ...
    generate_map_size_constants(&manifest_dir);
    generate_terrain_constants(&manifest_dir);
...
/// Generate the canonical land-cell size from the same data file TypeScript
/// consumes. Terrain LOS and other world-space kernels must not carry a
/// second, hand-authored copy of this scale.
fn generate_map_size_constants(manifest_dir: &Path) {
    let config_path = manifest_dir.join("../src/mapSizeConfig.json");
    ...
    let land_cell_size = json
        .pointer("/landCellSize")
        .and_then(Value::as_f64)
        .unwrap_or_else(|| {
            panic!(
                "missing numeric /landCellSize in {}",
                config_path.display()
            )
        });
    if !land_cell_size.is_finite() || land_cell_size <= 0.0 {
        panic!(
            "invalid /landCellSize in {}: expected positive finite number, got {}",
            config_path.display(),
            land_cell_size
        );
    }
    let generated = format!(
        "// Generated by rts-sim-wasm/build.rs from src/mapSizeConfig.json.\n\
         // Do not edit this file directly.\n\
         pub(crate) const MAP_LAND_CELL_SIZE: f64 = {land_cell_size:?};\n"
    );
    let out_path = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR is required"))
        .join("map_size_constants.rs");
...
fn generate_shared_sim_constants(manifest_dir: &Path) {
    ...
    // Continuous-time damping is the authored baseline. The motion kernel
    // converts it to the discrete critical coefficient for its actual fixed
    // step; using this value directly is only critical as dt approaches zero.
...
/// Generate the terrain floor / water-level constants from
/// src/game/sim/terrain/terrainConfig.json so the Rust samplers,
/// pathfinder, and unit kinetics can never drift from the TS side
/// (terrainConfig.ts computes WATER_LEVEL from the same two fields).
fn generate_terrain_constants(manifest_dir: &Path) {
    let config_path = manifest_dir.join("../src/game/sim/terrain/terrainConfig.json");
    rerun_if_changed(&config_path);
    let raw = fs::read_to_string(&config_path)
        .unwrap_or_else(|err| panic!("failed to read {}: {err}", config_path.display()));
    let json = parse_json(&raw, &config_path);
    ...
    let floor_y = json
        .pointer("/world/floorY")
        .and_then(Value::as_f64)
        .unwrap_or_else(|| panic!("missing numeric /world/floorY in {}", config_path.display()));
    let level_fraction = json
        .pointer("/water/levelFraction")
        .and_then(Value::as_f64)
        .unwrap_or_else(|| {
            panic!(
                "missing numeric /water/levelFraction in {}",
                config_path.display()
            )
        });
    if floor_y >= 0.0 {
        panic!(
            "invalid /world/floorY in {}: expected negative world floor, got {}",
            config_path.display(),
            floor_y
        );
    }
    if !(0.0..=1.0).contains(&level_fraction) {
        panic!(
            "invalid /water/levelFraction in {}: expected fraction in [0, 1], got {}",
            config_path.display(),
            level_fraction
        );
    }
    let water_level = floor_y * (1.0 - level_fraction);

    let generated = format!(
        "// Generated by rts-sim-wasm/build.rs from src/game/sim/terrain/terrainConfig.json.\n\
         // Do not edit this file directly.\n\
         #[allow(dead_code)]\n\
         pub(crate) const TERRAIN_TILE_FLOOR_Y: f64 = {floor_y:?};\n\
         pub(crate) const TERRAIN_WATER_LEVEL: f64 = {water_level:?};\n"
    );
    let out_path = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR is required"))
        .join("terrain_constants.rs");
    fs::write(&out_path, generated)
        .unwrap_or_else(|err| panic!("failed to write {}: {err}", out_path.display()));
}
...
fn generate_pathfinding_tuning(manifest_dir: &Path) {
    ...
    let allow_diagonal_neighbors =
        read_bool_field(&raw, "allowDiagonalNeighbors").unwrap_or_else(|| {
            panic!(
                "missing boolean allowDiagonalNeighbors in {}",
                config_path.display()
            )
        });
    ...
    let direct_path_max_cost_ratio = read_number_field(&raw, "directPathMaxCostRatio")
        .unwrap_or_else(|| {
            panic!(
                "missing numeric directPathMaxCostRatio in {}",
                config_path.display()
            )
        });
    if !direct_path_max_cost_ratio.is_finite() || direct_path_max_cost_ratio < 1.0 {
        panic!(
            "invalid directPathMaxCostRatio in {}: expected ratio >= 1, got {}",
            config_path.display(),
    ...
    let hierarchical_cluster_size_cells = read_number_field(&raw, "hierarchicalClusterSizeCells")
        .unwrap_or_else(|| {
            panic!(
                "missing numeric hierarchicalClusterSizeCells in {}",
                config_path.display()
            )
        });
    if hierarchical_cluster_size_cells < 2.0 || hierarchical_cluster_size_cells.fract() != 0.0 {
        panic!(
            "invalid hierarchicalClusterSizeCells in {}: expected integer >= 2, got {}",
            config_path.display(),
            hierarchical_cluster_size_cells
        );
    }
    ...
    generated.push_str(&format!(
        "const PATHFINDING_DIRECT_PATH_MAX_COST_RATIO: f32 = {:?};\n",
    ...
    generated.push_str(&format!(
        ...
        hierarchical_cluster_size_cells as i32
    ));
...
fn read_bool_field(raw: &str, field: &str) -> Option<bool> {
    let key = format!("\"{field}\"");
    let key_start = raw.find(&key)?;
    let after_key = &raw[key_start + key.len()..];
    let colon = after_key.find(':')?;
