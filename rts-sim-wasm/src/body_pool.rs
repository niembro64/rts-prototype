...
pub(crate) fn compute_airborne_waypoint_steer(
    ...
    let inv_distance = 1.0 / distance;
    let goal_dir_x = dx * inv_distance;
    let goal_dir_y = dy * inv_distance;
...
/// Body A's contact data and the broadphase cell window to scan for its
/// partners.
///
/// All four sphere resolve passes (hash/dense x all/active) open with exactly
/// this computation; only what they do inside the cell window differs. Sharing
/// it keeps the four passes from drifting into different broadphase windows,
/// which would make contacts appear or vanish depending on which pass ran.
struct SphereBroadphaseQuery {
    ar: f64,
    a_inv_mass: f64,
    a_restitution: f64,
    query_min_cx: i32,
...
fn sphere_broadphase_query(
    ...
    max_cy: i32,
    max_cz: i32,
) -> SphereBroadphaseQuery {
    let slot_a = sphere_slots[i] as usize;
    let ar = p.radius[slot_a];
    // A global `max_radius * 2` range is correct but expensive: one
    // queen-class body makes every small unit scan a 5x5x5 neighborhood. For
    // body A, any possible B has radius <= max_radius, so `ar + max_radius` is
    // the same conservative broadphase bound with fewer impossible bucket
    // probes.
    ...
    SphereBroadphaseQuery {
        ar,
        a_inv_mass: p.inv_mass[slot_a],
        a_restitution: p.restitution[slot_a],
        query_min_cx: (acx - range).max(min_cx),
        query_max_cx: (acx + range).min(max_cx),
        query_min_cy: (acy - range).max(min_cy),
        query_max_cy: (acy + range).min(max_cy),
        query_min_cz: (acz - range).max(min_cz),
        query_max_cz: (acz + range).min(max_cz),
    }
}

/// Choose the dense-grid or hashed broadphase and fill its buckets.
///
/// Both sphere-resolve entry points make the same choice from the same cell
/// bounds and then bin every body the same way; returning the shape alongside
/// the decision lets each caller pick the matching resolve pass. Keeping the
/// choice in one place means the binning and the pass selection can never
/// disagree about which structure holds this frame's buckets.
#[allow(clippy::too_many_arguments)]
fn sphere_contact_fill_buckets(
    scratch: &mut SphereResolveScratch,
    count: usize,
    gen: u32,
    min_cx: i32,
    min_cy: i32,
...
pub fn pool_resolve_sphere_sphere(
    ...
    let (dense_shape, use_dense) = sphere_contact_fill_buckets(
        scratch, count, gen, min_cx, min_cy, min_cz, max_cx, max_cy, max_cz,
    );
